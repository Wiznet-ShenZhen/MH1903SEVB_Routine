...
uint8_t serverIP[4] = { 192,168,1,21};
...
void TCP_Client()
{
	...
	case SOCK_ESTABLISHED:
		...
		len = getSn_RX_RSR(TCP_CLIENT_SOCKET);

		if (len > 0)
		{
			recv(TCP_CLIENT_SOCKET, buffer, len);

			printf("Receive : %s\r\n", buffer);

			send(TCP_CLIENT_SOCKET, buffer, len);
		...
		if (USART_RX_STA & 0x8000)
		{
			...
			send(TCP_CLIENT_SOCKET, USART_RX_BUF, len);
	...
	case SOCK_CLOSED:
		...
		socket(TCP_CLIENT_SOCKET, Sn_MR_TCP, localPort, Sn_MR_ND);
